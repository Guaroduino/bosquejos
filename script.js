document.addEventListener('DOMContentLoaded', () => {
    ...
    function handleZoom(e) {
        ...
        const newZoom = Math.min(Math.max(zoom + (zoomDirection * ZOOM_STEP), MIN_ZOOM), MAX_ZOOM);
    ...
    // Add touch gesture zoom
    let lastTouchDistance = 0;
    fabricCanvas.on('touch:gesture', function(e) {
        if (e.self && e.self.target) {
            if (e.self.type === 'pinch') {
                // Calculate distance between two touch points
                const touch1 = e.self.pointers[0];
                const touch2 = e.self.pointers[1];
    ...
    // Reset lastTouchDistance when touch ends
    ...
    // Define resizeCanvas before using it
    ...
    // DXF Import Elements
    const loadDxfInput = document.createElement('input');
    loadDxfInput.type = 'file';
    loadDxfInput.id = 'load-dxf-input';
    loadDxfInput.accept = '.dxf';
    loadDxfInput.style.display = 'none';
    document.body.appendChild(loadDxfInput);

    const loadDxfButton = document.createElement('button');
    loadDxfButton.id = 'load-dxf-button';
    loadDxfButton.textContent = 'Import DXF';
    loadDxfButton.className = 'tool-button';
    loadDxfButton.title = 'Import DXF file';
    toolbar.insertBefore(loadDxfButton, document.getElementById('clear-canvas'));

    loadDxfButton.addEventListener('click', () => {
        loadDxfInput.click();
    });

    loadDxfInput.addEventListener('change', (event) => {
        const file = event.target.files[0];
        if (!file) return;
        ...
        reader.onload = (e) => {
            try {
                // Parse the DXF and convert to SVG
                const dxfString = e.target.result;
                const helper = new window.Dxf.Helper(dxfString);
                const svgString = helper.toSVG();
                // Import the SVG string using the refactored function
                importSvgString(svgString);
            } catch (err) {
                console.error("Error parsing DXF:", err);
                alert("Error parsing DXF file. The file might be corrupted or contain unsupported elements.");
            }
            loadDxfInput.value = '';
        };
        reader.readAsText(file);
    ...
    // Refactor SVG import logic into a reusable function
    function importSvgString(svgString) {
        // Inline styles and remove <style> blocks if needed
        svgString = inlineSvgStyles(svgString);
        ...
        const svgDoc = parser.parseFromString(svgString, 'image/svg+xml');
        if (svgDoc.documentElement.nodeName !== 'svg') {
            alert("Invalid SVG file generated from DXF.");
            showStatus("");
            return;
        }
        ...
        // Load SVG using Fabric.js (reuse your existing logic)
        fabric.loadSVGFromString(cleanSvgString, (objects, options) => {
            if (!objects || objects.length === 0) {
                alert("Could not load SVG or SVG is empty/unsupported.");
                showStatus("");
